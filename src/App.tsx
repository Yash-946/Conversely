...
const App = () => {
  ...
  const handleBoxClick = (id) => {
    if (!clickedBoxes.includes(`box-${id}`)) {
      setClickedBoxes((prev) => [...prev, `box-${id}`]);
    } else {
      setClickedBoxes((prev) => prev.filter((boxId) => boxId !== `box-${id}`));
    }
  };
        ...
        <div className="grid grid-cols-3 gap-2">
          {matrix.map((item) => (
            <div
              ...
              onDoubleClick={() => alert(`Double-clicked box ${item.id}`)}
